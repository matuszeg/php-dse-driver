...
class ExecutionOptionsTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAllowsRetrievingSettingsByName()
    {
        $options = new ExecutionOptions(array(
            'consistency'        => \Dse::CONSISTENCY_ANY,
            'serial_consistency' => \Dse::CONSISTENCY_LOCAL_SERIAL,
            'page_size'          => 15000,
            'timeout'            => 15,
            'arguments'          => array('a', 1, 'b', 2, 'c', 3)
        ));

        $this->assertEquals(\Dse::CONSISTENCY_ANY, $options->consistency);
        $this->assertEquals(\Dse::CONSISTENCY_LOCAL_SERIAL, $options->serialConsistency);
