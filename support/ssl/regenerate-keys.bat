...
SET "SERVER_CERTIFICATE=!ABSOLUTE_BATCH_DIRECTORY!\cassandra.crt"
...
SET PASSPHRASE=cassandra

SET "SERVER_KEYSTORE=!ABSOLUTE_BATCH_DIRECTORY!\keystore.jks"
SET SERVER_KEYSTORE_PASSPHRASE=cassandra
SET "SERVER_TRUSTSTORE=!ABSOLUTE_BATCH_DIRECTORY!\truststore.jks"
SET SERVER_TRUSTSTORE_PASSPHRASE=cassandra
SET "CLIENT_KEYSTORE=%TEMP%\driver.keystore"
SET CLIENT_KEYSTORE_PASSPHRASE=cassandra
...
!KEYTOOL! -exportcert -noprompt ^
  ...
  -keystore "!SERVER_KEYSTORE!" ^
  -storepass "!SERVER_KEYSTORE_PASSPHRASE!" ^
  -file "!SERVER_CERTIFICATE!"> NUL 2>&1
IF NOT !ERRORLEVEL! EQU 0 (
  ECHO FAILED!
...
IF NOT !ERRORLEVEL! EQU 0 (
  ECHO FAILED!
  ECHO Unable to Generate Server Certificate [PEM]: !ERRORLEVEL!
  EXIT /B !EXIT_CODE_KEYTOOL_COMMAND_FAILURE!
)
