...
SET BUILD_PACKAGE_PREFIX=dse-php-driver
...
SET CPP_DRIVER_DIRECTORY=cpp-driver-dse
...
SET PHP_7_0_BRANCH_TAG_VERSION=php-7.0.17
...
SET LIBSSH2_PHP_BINDINGS_URL=https://git.php.net/repository/pecl/networking/ssh2.git
REM libssh2 bindings have two different branches and SHA for PHP versions
SET LIBSSH2_PHP_5_6_BRANCH_TAG_VERSION=0.13
SET LIBSSH2_PHP_5_6_BRANCH_TAG=php5
SET LIBSSH2_PHP_5_6_SHA_CHANGESET=9a6c6c4
SET LIBSSH2_PHP_7_0_BRANCH_TAG_VERSION=1.0
SET LIBSSH2_PHP_7_0_BRANCH_TAG=master
SET LIBSSH2_PHP_7_0_SHA_CHANGESET=30b9e12
...
SET "ABSOLUTE_DEPENDENCIES_LIBSSH2_SOURCE_DIRECTORY=%ABSOLUTE_BUILD_DIRECTORY%\%DEPENDENCIES_DIRECTORY%\%DEPENDENCIES_SOURCE_DIRECTORY%\%LIBSSH2_DIRECTORY%"
SET "ABSOLUTE_DEPENDENCIES_LIBSSH2_LIBRARIES_DIRECTORY=%ABSOLUTE_BUILD_DIRECTORY%\%DEPENDENCIES_DIRECTORY%\%DEPENDENCIES_LIBRARIES_DIRECTORY%\%LIBSSH2_DIRECTORY%"
...
SET "LOG_CPP_DRIVER_BUILD=%ABSOLUTE_LOG_DIRECTORY%\cpp-driver-dse.log"
...
IF NOT [%1] == [] (
  ...
  IF "!ARGUMENT!" == "!ARGUMENT_PHP_VERSION!" (
    ...
    IF [%2] == [] (
      ...
      IF NOT "%2" == "5.6" (
        IF NOT "%2" == "7.0" (
          ECHO Invalid Version: Version not within range [5.6 and 7.0]
          EXIT /B !EXIT_CODE_INVALID_VERSION!
        )
      )
  ...
  REM Enable the use of Boost atomics library in the cpp-driver-dse
...
REM Remove versions of Visual Studio that are not applicable for PHP DSE driver
...
IF "!ENABLE_BUILD_PACKAGES!" == "!FALSE!" (
  ...
  REM Ensure Kerberos SDK is available (32 and 64 bit)
  FOR %%B IN (!AVAILABLE_TARGET_ARCHITECTURES!) DO (
    SET "KERBEROS_SDK_DIRECTORY=!PROGRAMFILES!\MIT\Kerberos"
    IF %%B EQU !ARCHITECTURE_32BIT! (
      IF !SYSTEM_ARCHITECTURE! EQU !ARCHITECTURE_64BIT! (
        SET "KERBEROS_SDK_DIRECTORY=!PROGRAMFILES(X86)!\MIT\Kerberos"
      )
    )
...
IF !ENABLE_CLEAN_BUILD! EQU !TRUE! (
  ...
  CALL :CLEANDIRECTORY "!ABSOLUTE_DEPENDENCIES_CPP_DRIVER_SOURCE_DIRECTORY!" "Cleaning cpp-driver-dse submodule directory"
  ECHO.
)
...
REM Initialize and update the cpp-driver-dse submodule
IF NOT EXIST "!ABSOLUTE_DEPENDENCIES_CPP_DRIVER_SOURCE_DIRECTORY!\include\dse.h" (
  PUSHD "!ABSOLUTE_BATCH_DIRECTORY!\.." > NUL
  ECHO Update cpp-driver-dse submodule > "!LOG_CPP_DRIVER_BUILD!"
  ECHO | SET /P="Update cpp-driver-dse submodule ... "
  !GIT! submodule update --init --recursive !ABSOLUTE_DEPENDENCIES_CPP_DRIVER_SOURCE_DIRECTORY! >> "!LOG_CPP_DRIVER_BUILD!" 2>&1
  IF !ERRORLEVEL! EQU 0 (
    ECHO done.
    POPD
  ) ELSE (
...
REM Clone libssh2, libssh2 php bindings and checkout the appropriate tag
IF !ENABLE_TEST_CONFIGURATION! EQU !TRUE! (
  IF NOT EXIST "!ABSOLUTE_DEPENDENCIES_LIBSSH2_SOURCE_DIRECTORY!" (
    ECHO Cloning libssh2 !LIBSSH2_BRANCH_TAG_VERSION! > "!LOG_LIBSSH2_BUILD!"
    ECHO | SET /P=Cloning libssh2 !LIBSSH2_BRANCH_TAG_VERSION! ... 
    !GIT! clone --depth 1 --branch !LIBSSH2_BRANCH_TAG_VERSION! --single-branch !LIBSSH2_REPOSITORY_URL! "!ABSOLUTE_DEPENDENCIES_LIBSSH2_SOURCE_DIRECTORY!" >> "!LOG_LIBSSH2_BUILD!" 2>&1
    IF !ERRORLEVEL! EQU 0 (
      ECHO done.
    ) ELSE (
      ECHO FAILED!
...
FOR %%A IN (!PHP_VERSIONS!) DO (
  ...
  REM Determine if libssh2 bindings extension should be cloned
  IF !ENABLE_TEST_CONFIGURATION! EQU !TRUE! (
    REM Determine the appropriate branch/tag and SHA
    IF "%%A" == "7_0" (
      SET LIBSSH2_PHP_BRANCH_TAG_VERSION=!LIBSSH2_PHP_7_0_BRANCH_TAG_VERSION!
      SET LIBSSH2_PHP_SHA_CHANGESET=!LIBSSH2_PHP_7_0_SHA_CHANGESET!
    )
    IF "%%A" == "5_6" (
      SET LIBSSH2_PHP_BRANCH_TAG_VERSION=!LIBSSH2_PHP_5_6_BRANCH_TAG_VERSION!
      SET LIBSSH2_PHP_SHA_CHANGESET=!LIBSSH2_PHP_5_6_SHA_CHANGESET!
    )

    REM Move to the PHP source extension directory and clone the repo
    IF NOT EXIST "!ABSOLUTE_DEPENDENCIES_PHP_SOURCE_DIRECTORY!\ext\ssh2" (
      PUSHD "!ABSOLUTE_DEPENDENCIES_PHP_SOURCE_DIRECTORY!\ext" > NUL
      ECHO Cloning libssh2 bindings !LIBSSH2_PHP_BRANCH_TAG_VERSION! [!LIBSSH2_PHP_SHA_CHANGESET!] for PHP >> "!LOG_DRIVER_BUILD!"
      ECHO | SET /P="Cloning libssh2 bindings !LIBSSH2_PHP_BRANCH_TAG_VERSION! [!LIBSSH2_PHP_SHA_CHANGESET!] for PHP ... "
      !GIT! clone !LIBSSH2_PHP_BINDINGS_URL! ssh2 >> "!LOG_DRIVER_BUILD!" 2>&1
      IF !ERRORLEVEL! EQU 0 (
        PUSHD ssh2 > NUL
        !GIT! reset !LIBSSH2_PHP_SHA_CHANGESET! --hard >> "!LOG_DRIVER_BUILD!" 2>&1
        IF !ERRORLEVEL! EQU 0 (
          POPD > NUL
          ECHO done.
        ) ELSE (
...
IF !ENABLE_BUILD_PACKAGES! EQU !FALSE! (
  ...
  REM Determine if the cpp-driver-dse needs to be built
  ...
  IF EXIST "!ABSOLUTE_DEPENDENCIES_LIBSSH2_SOURCE_DIRECTORY!" (
    IF NOT EXIST "!ABSOLUTE_DEPENDENCIES_LIBSSH2_LIBRARIES_DIRECTORY!" (
      CALL :BUILDLIBSSH2 "!ABSOLUTE_DEPENDENCIES_LIBSSH2_SOURCE_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_LIBSSH2_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_OPENSSL_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_ZLIB_LIBRARIES_DIRECTORY!" !TARGET_ARCHITECTURE! !VISUAL_STUDIO_INTERNAL_SHORTHAND_VERSION! !FALSE! "!LOG_LIBSSH2_BUILD!"
      IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
    )
  )
  ...
  IF NOT EXIST "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!" (
    CALL :BUILDDRIVER "!ABSOLUTE_DEPENDENCIES_PHP_SOURCE_DIRECTORY!" "!ABSOLUTE_BATCH_DIRECTORY!" "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!" !ENABLE_TEST_CONFIGURATION! !ENABLE_THREAD_SAFETY! !BUILD_TYPE! "!ABSOLUTE_DEPENDENCIES_LIBICONV_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_LIBSSH2_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_LIBXML2_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_CPP_DRIVER_LIBRARIES_DIRECTORY!" "!KERBEROS_SDK_LIBRARY_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_LIBUV_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_OPENSSL_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_ZLIB_LIBRARIES_DIRECTORY!" "!ABSOLUTE_DEPENDENCIES_MPIR_LIBRARIES_DIRECTORY!" "!LOG_DRIVER_BUILD!"
    IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
    ECHO extension=php_dse.dll >> "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!\php.ini"

    REM Configure PHP for use with OpenSSL and Kerberos runtime libraries
    ...
    XCOPY /Y /E "!KERBEROS_SDK_RUNTIME_DIRECTORY!\*!TARGET_ARCHITECTURE!.dll" "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!" >> "!PHP_DRIVER_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !PHP_DRIVER_LOG_FILENAME! for more details
      EXIT /B !EXIT_CODE_BUILD_DRIVER_FAILED!
  ...
  FOR %%A IN (!SUPPORTED_PHP_VERSIONS!) DO (
    ...
    IF !VISUAL_STUDIO_FOUND! EQU !TRUE! (
      ...
      FOR %%B IN (!AVAILABLE_TARGET_ARCHITECTURES!) DO (
        ...
        REM Ensure the proper Kerberos SDK is assigned
        SET "KERBEROS_SDK_DIRECTORY=!PROGRAMFILES!\MIT\Kerberos"
        IF %%B EQU !ARCHITECTURE_32BIT! (
          IF !SYSTEM_ARCHITECTURE! EQU !ARCHITECTURE_64BIT! (
            SET "KERBEROS_SDK_DIRECTORY=!PROGRAMFILES(X86)!\MIT\Kerberos"
        ...
        CALL :BUILDDRIVER "!ABSOLUTE_DEPENDENCIES_PHP_SOURCE_DIRECTORY!" "!ABSOLUTE_BATCH_DIRECTORY!" "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!" !FALSE! !TRUE! !BUILD_TYPE_RELEASE! "" "" "" "!ABSOLUTE_DRIVER_DEPENDENCY_CPP_DRIVER_PACKAGE_INSTALLATION_DIRECTORY!" "!KERBEROS_SDK_LIBRARY_DIRECTORY!" "!ABSOLUTE_CPP_DRIVER_DEPENDENCY_LIBUV_PACKAGE_INSTALLATION_DIRECTORY!" "!ABSOLUTE_CPP_DRIVER_DEPENDENCY_OPENSSL_PACKAGE_INSTALLATION_DIRECTORY!" "!ABSOLUTE_CPP_DRIVER_DEPENDENCY_ZLIB_PACKAGE_INSTALLATION_DIRECTORY!" "!ABSOLUTE_DRIVER_DEPENDENCY_MPIR_PACKAGE_INSTALLATION_DIRECTORY!" "!LOG_DRIVER_BUILD!"
        IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
        MKDIR "!ABSOLUTE_PACKAGES_DIRECTORY!\!DRIVER_PACKAGE_INSTALLATION_DIRECTORY!\ts\doc"
        SET "ABSOLUTE_DRIVER_LIBRARY=!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!\php_dse.dll"
        IF NOT EXIST "!ABSOLUTE_DRIVER_LIBRARY!" SET "ABSOLUTE_DRIVER_LIBRARY=!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!\ext\php_dse.dll"
...
REM Build the C/C++ DSE driver library
REM
REM @param source-directory Location of driver source
REM @param target-compiler Target compiler to use for compiling the
REM                        cpp-driver-dse
REM @param install-directory Installation location of the cpp-driver-dse
...
:BUILDCPPDRIVER [source-directory] [target-compiler] [install-directory] [build-type] [target-architecture] [use-boost-atomic] [log-filename]
  ...
  ECHO | SET /P="Building and installing cpp-driver-dse ... "
...
:BUILDMPIR [source-directory] [install-directory] [build-type] [target-architecture] [visual-studio-version] [log-filename]
  ...
  IF !MPIR_VISUAL_STUDIO_VERSION! EQU 2012 SET MPIR_VISUAL_STUDIO_VERSION=11.0
  IF !MPIR_VISUAL_STUDIO_VERSION! EQU 2015 SET MPIR_VISUAL_STUDIO_SOLUTION_DIRECTORY=build.vc14
  IF !MPIR_VISUAL_STUDIO_VERSION! EQU 2015 SET MPIR_VISUAL_STUDIO_VERSION=14.0
  REM Build MPIR
  ...
  ECHO !MSBUILD! /P:Configuration=!MPIR_BUILD_TYPE! /P:Platform=!MPIR_PLATFORM_ARCHITECTURE!;VisualStudioVersion=!MPIR_VISUAL_STUDIO_VERSION! /CLP:NoSummary;NoItemAndPropertyList;Verbosity=minimal /NOLOGO !MPIR_VISUAL_STUDIO_SOLUTION_DIRECTORY!\lib_mpir_gc\lib_mpir_gc.vcxproj >> "!MPIR_LOG_FILENAME!" 2>&1
  !MSBUILD! /P:Configuration=!MPIR_BUILD_TYPE! /P:Platform=!MPIR_PLATFORM_ARCHITECTURE!;VisualStudioVersion=!MPIR_VISUAL_STUDIO_VERSION! /CLP:NoSummary;NoItemAndPropertyList;Verbosity=minimal /NOLOGO !MPIR_VISUAL_STUDIO_SOLUTION_DIRECTORY!\lib_mpir_gc\lib_mpir_gc.vcxproj >> "!MPIR_LOG_FILENAME!" 2>&1
  IF NOT !ERRORLEVEL! EQU 0 (
    ECHO FAILED!
  ...
  PUSHD !MPIR_VISUAL_STUDIO_SOLUTION_DIRECTORY!\lib_mpir_gc\!MPIR_PLATFORM_ARCHITECTURE!\!MPIR_BUILD_TYPE! > NUL
  ...
  FOR %%A IN (!MPIR_BINARY_FILES!) DO (
    ECHO !CWD! >> "!MPIR_LOG_FILENAME!" 2>&1
...
REM @param libssh2-library-directory Library directory for libssh2
REM @param libxml2-library-directory Library directory for libxml2
REM @param cpp-driver-library-directory Library directory for cpp-driver-dse
REM @param kerberos-library-directory Library directory for Kerberos
...
REM @param log-filename Absolute path and filename for log output
:BUILDDRIVER [php-source-directory] [extension-source-directory] [install-directory] [enable-test-configuration] [enable-thread-safety] [build-type] [libiconv-library-directory] [libssh2-library-directory] [libxml2-library-directory] [cpp-driver-library-directory] [kerberos-library-directory] [libuv-library-directory] [openssl-library-directory] [zlib-library-directory] [mpir-library-directory] [log-filename]
  REM Create php-driver variables from arguments
  ...
  IF NOT EXIST "!PHP_DRIVER_PHP_SOURCE_DIRECTORY!\..\deps" (
    ...
    XCOPY /Y /E "!PHP_DRIVER_KERBEROS_LIBRARY_DIRECTORY!\*.*" "!PHP_DRIVER_PHP_SOURCE_DIRECTORY!\..\deps\lib" >> "!PHP_DRIVER_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !PHP_DRIVER_LOG_FILENAME! for more details
      RMDIR /S /Q "!PHP_DRIVER_PHP_SOURCE_DIRECTORY!\..\deps" > NUL
    ...
    IF "!PHP_DRIVER_ENABLE_TEST_CONFIGURATION!" == "!TRUE!" (
      ...
      XCOPY /Y /E "!PHP_DRIVER_LIBSSH2_LIBRARY_DIRECTORY!\*.*" "!PHP_DRIVER_PHP_SOURCE_DIRECTORY!\..\deps" >> "!PHP_DRIVER_LOG_FILENAME!" 2>&1
      IF NOT !ERRORLEVEL! EQU 0 (
        ECHO FAILED!
        ECHO 	See !PHP_DRIVER_LOG_FILENAME! for more details
        RMDIR /S /Q "!PHP_DRIVER_PHP_SOURCE_DIRECTORY!\..\deps" > NUL
  ...
  SET "DRIVER_CONFIGURE_COMMAND_LINE=--with-prefix=^"!PHP_DRIVER_INSTALLATION_DIRECTORY!^" --disable-all --enable-cli --enable-com-dotnet --enable-session --enable-zlib --with-gmp --with-openssl --enable-dse=shared"
  ...
  IF "!PHP_DRIVER_ENABLE_TEST_CONFIGURATION!" == "!TRUE!" (
    SET "DRIVER_CONFIGURE_COMMAND_LINE=!DRIVER_CONFIGURE_COMMAND_LINE! --enable-phar --enable-json --enable-filter --enable-hash --enable-ctype --enable-mbstring --enable-mbregex --enable-mbregex-backtrack --enable-tokenizer --enable-zip --with-dom --with-iconv=^"!PHP_DRIVER_LIBICONV_LIBRARY_DIRECTORY!^" --with-libxml --with-ssh2 --with-xml"
  )
